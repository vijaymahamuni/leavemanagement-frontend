...
const Login = () => {
  ...
  async function loginAndUpdateMessages(email, password) {
    try {
      const userCredential = await signInWithEmailAndPassword(auth, email, password);
      console.log("User signed in: ", userCredential.user.uid);
      const targetId=userCredential.user.uid;
      const chatCollectionGroupRef = collectionGroup(db, "chat");
      const querySnapshot = await getDocs(chatCollectionGroupRef);
      for (const docSnapshot of querySnapshot.docs) {
        const docData = docSnapshot.data();
        if (docData.to === targetId) {
          const chatDocRef = doc(db, docSnapshot.ref.path);
  ...
  const handleSubmit = async (e) => {
    ...
    loginAndUpdateMessages(newemail, newpass);
    ...
    console.log("result of auth", result.user.uid)
    setloggedOut(true)
  }
  React.useEffect(() => {
