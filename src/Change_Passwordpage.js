...
function Change_Passwordpage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (oldPassword.length < 6 || newPassword.length < 6) {
      Swal.fire({
        title: 'Alert',
        text: 'Passwords should be at least 6 characters long!',
        icon: 'info'
      });
      return;
    }
    ...
    if (!strongPasswordRegex.test(newPassword)) {
      ...
      Swal.fire({
        title: 'Alert',
        text: 'New password should contain at least one uppercase letter, one lowercase letter, one number, and one special character.'
        ,
        icon: 'info'
      });
      return;
    ...
    if (oldPassword != proPassword) {
      Swal.fire({
        title: 'error',
        text: 'The Old Password is incorrect,Try again..!',
        icon: 'error'
      });
    }
    else {
